...
${parameters.map(parameter => `
		...
		//#region PHP State Handler Setup
		{
			const sourceStateHandler = path.join(sourceToTemplate, 'init', 'php', 'States', 'StateHandlerInterface.php');
			const destStateDir = path.join('modules', 'php', 'States');

			if (fs.existsSync(sourceStateHandler)) {
				builder.watchCommand("PHP State Handler Setup", sourceStateHandler, () => {

					// Read template content
					let content = fs.readFileSync(sourceStateHandler, 'utf8');

					// Replace placeholders
					content = content.replace(/___YourGameName___/g, config.YourGameName);

					// Create destination directory if needed
					if (!fs.existsSync(destStateDir)) {
						fs.mkdirSync(destStateDir, { recursive: true });
					}

					// Write modified file
					const destPath = path.join(destStateDir, 'StateHandlerInterface.php');
					fs.writeFileSync(destPath, content);

					console.log("\t"+`Updated StateHandlerInterface.php for ${config.YourGameName}`);
				});

				// Initial run
				builder.commands.push({
					message: "Setting up PHP State Handler interface",
					callback: () => {
						if (!fs.existsSync(destStateDir)) {
							fs.mkdirSync(destStateDir, { recursive: true });
						}
						// Trigger the watch command callback immediately
						builder.commands.find(c => c.message === "PHP State Handler Setup").callback();
					}
				});
			} else {
				console.warn(`State handler template not found at: ${sourceStateHandler}`);
			}
		}
		//#endregion
		...
		//#region Generate State Handlers
		{
			const statesDir = path.join('modules', 'php', 'States');
			if (!fs.existsSync(statesDir)) {
				fs.mkdirSync(statesDir, { recursive: true });
			}

			const upperCamelName = config.YourGameName;

			for (const key in statesJSON) {
				const stateDef = statesJSON[key];
				const stateName = stateDef.name;
				// Skip game states and special states
				if (stateName.endsWith('Game') || ['gameSetup', 'gameEnd'].includes(stateName)) continue;

				const className = `${stateName.replace(/(?:^|_)(\w)/g, (_, c) => c.toUpperCase())}StateHandler`;
				const fileName = `${className}.php`;
				const filePath = path.join(statesDir, fileName);

				// Get possible actions with parameters
				const actions = stateDef.possibleactions || [];
				const actionCases = actions.map(actionName => {
					const parameters = possibleActions.get(actionName) || [];
					return `            case '${actionName}':
                self::${actionName}(\$game${parameters.length ? ', ' : ''}${parameters.map((_, i) => `\$args[${i}]`).join(', ')});
                break;`;
				}).join('\n');

				// Generate handle function content
				const handleFunctionContent = `/* AUTOGENERATED_HANDLE_START - DO NOT REMOVE THIS COMMENT */
...
${actionCases}
    ...
    /* AUTOGENERATED_HANDLE_END - DO NOT REMOVE THIS COMMENT */`;

				// Generate action methods
				const actionMethods = actions.map(actionName => {
					const parameters = possibleActions.get(actionName) || [];
					const phpParams = parameters.map(p => {
						const phpType = pType(p);
						return `${phpType} \$${underscoreToCamel(p.name)}`;
					}).join(', ');

					return `
    /* AUTOGENERATED_METHOD - ${actionName} */
    private static function ${actionName}(Game \$game${parameters.length ? ', ' : ''}${phpParams}): void 
    {
        // Add your action logic here
				...
				if (fs.existsSync(filePath)) {
					...
					content = content.replace(
						/(\/\* AUTOGENERATED_HANDLE_START.*?\*\/)(.*?)(\/\* AUTOGENERATED_HANDLE_END.*?\*\/)/gs,
						`${handleFunctionContent}`
					);

					// Add missing action methods
					actions.forEach(actionName => {
						if (!content.includes(`function ${actionName}(`)) {
							const methodContent = actionMethods.find(m => m.includes(`function ${actionName}(`));
							content = content.replace(
								/(\}\s*$)/,
								`\n${methodContent}\n$1`
							);
					...
					// Create new file
					const handlerContent = `<?php

namespace Bga\\Games\\${upperCamelName}\\States;

use Bga\\Games\\${upperCamelName}\\Game;
use BgaUserException;
use BgaVisibleSystemException;

class ${className} implements StateHandlerInterface
{
    ${handleFunctionContent}
${actionMethods}
...
`;
				...
				console.log("\t"+`Updated state handler: ${fileName}`);
			}
		}
		//#endregion
